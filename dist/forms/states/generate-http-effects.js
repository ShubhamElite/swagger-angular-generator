...
function generateHttpEffects(config, name, simpleName, actionClassNameBase, formSubDirName, paramGroups) {
    ...
    content += "}\n";
    var effectsFileName = path.join(formSubDirName, "states", "effects.ts");
...
function getEffectDefinition(actionClassNameBase, name, simpleName, hasParams) {
    var startActionPayloadDefinition = getStartActionPayloadDefinition(hasParams);
    var res = utils_1.indent("@Effect()\n");
    ...
    var actionParam = hasParams ? 'action: actions.Start' : '';
